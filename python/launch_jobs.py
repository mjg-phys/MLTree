...
parser.add_argument("--datasets", type=str, dest="datasets", required=False, default="datasets_jets.json", metavar = "<datasets.json>", help="JSON file specifying the input and output datasets.")
parser.add_argument("--nFilesPerJob", required=False, type=int, dest="nFilesPerJob", default=2, help="Number of files per job")
...
setup = "python runCA.py --filesInput=%IN"
...
config = "--useAthenaPackage --outputs=\"mltree.root\" --nFilesPerJob=5"

comFirst = "prun --exec=\"{}\" --outDS={} --inDS={} {}"
comLater = "prun --exec=\"{}\" --outDS={} --inDS={} --libDS LAST {}"

print()

# Submit jobs to the grid with pathena
# https://twiki.cern.ch/twiki/bin/view/PanDA/PandaAthena
for i, inDS, outDS in zip(range(len(inDSs)), inDSs, outDSs):
